...
public class Resources
{
    ...
    internal class FileAssemblyInfo
    {
        ...
        private void SplitFileAndAssemblyNames(string originalFileName)
        {
            ...
            var separatorIndex = originalFileName.IndexOf(FileAssemblySeparator);
        ...
        public bool IsAssemblyQualified
        {
            get
            {
                if (AssemblyName == null || AssemblyName.Trim().Length == 0) return false;
    ...
    {
        var pos = filePath.IndexOf(PROTOCOL_SEPARATOR);
    ...
    public static XmlDocument GetAsXmlDocument(XmlNode node, NameValueCollection properties)
    {
        ...
        {
            var ressource = NodeUtils.ParsePropertyTokens(node.Attributes["resource"].Value, properties);
            xmlDocument = GetResourceAsXmlDocument(ressource);
        }
        else if (node.Attributes["url"] != null)
        {
            var url = NodeUtils.ParsePropertyTokens(node.Attributes["url"].Value, properties);
            xmlDocument = GetUrlAsXmlDocument(url);
        }
        else if (node.Attributes["embedded"] != null)
        {
            var embedded = NodeUtils.ParsePropertyTokens(node.Attributes["embedded"].Value, properties);
    ...
    public static XmlDocument GetEmbeddedResourceAsXmlDocument(string resource)
    {
        ...
        var fileInfo = new FileAssemblyInfo(resource);
        if (fileInfo.IsAssemblyQualified)
        {
            Assembly assembly = null;
            assembly = Assembly.Load(fileInfo.AssemblyName);
            var stream = assembly.GetManifestResourceStream(fileInfo.ResourceFileName);
            // JIRA - IBATISNET-103 
        ...
        else
        {
            ...
            {
                var stream = assembly.GetManifestResourceStream(fileInfo.FileName);
                if (stream != null)
                {
                    try
                    {
