...
public class SetAccessorFactory : ISetAccessorFactory
{
    ...
    {
        var key = new StringBuilder(targetType.FullName).Append(".").Append(name).ToString();
        ...
        lock (_syncObject)
        {
            ...
            {
                // Property
                var reflectionCache = ReflectionInfo.GetInstance(targetType);
                var memberInfo = reflectionCache.GetSetter(name);
    ...
    {
        var reflectionCache = ReflectionInfo.GetInstance(targetType);
        var propertyInfo = (PropertyInfo)reflectionCache.GetSetter(propertyName);
    ...
    {
        var reflectionCache = ReflectionInfo.GetInstance(targetType);
        var fieldInfo = (FieldInfo)reflectionCache.GetSetter(fieldName);

        if (fieldInfo.IsPublic) return new DelegateFieldSetAccessor(targetType, fieldName);
    ...
    private ISetAccessor CreateFieldAccessor(Type targetType, string fieldName)
    {
        ...
        if (fieldInfo.IsPublic) return new EmitFieldSetAccessor(targetType, fieldName, _assemblyBuilder, _moduleBuilder);
