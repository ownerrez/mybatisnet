...
public class TypeHandlerFactory
{
    ...
    {
        if (_typeAliasMaps.Contains(name)) return (TypeAlias)_typeAliasMaps[name];
    ...
    {
        Type type = null;
        var typeAlias = GetTypeAlias(className);
    ...
    {
        if (_typeAliasMaps.Contains(key)) throw new DataMapperException(" Alias name conflict occurred.  The type alias '" + key + "' is already mapped to the value '" + typeAlias.ClassName + "'.");
    ...
    {
        if (type.IsEnum) return GetPrivateTypeHandler(typeof(Enum), dbType);
    ...
    {
        var dbTypeHandlerMap = (IDictionary)_typeHandlerMap[type];
        ITypeHandler handler = null;
        ...
        if (dbTypeHandlerMap != null)
        {
            ...
            if (handler == null) throw new DataMapperException(string.Format("Type handler for {0} not registered.", type.Name));
        }
    ...
    {
        var map = (HybridDictionary)_typeHandlerMap[type];
        if (map == null)
        {
        ...
        if (dbType == null)
        {
            if (_logger.IsInfoEnabled)
            {
                ...
                var oldTypeHandler = (ITypeHandler)map[NULL];

                if (oldTypeHandler != null)
                {
                    // the replacement will always(?) be a CustomTypeHandler
                    var customTypeHandler = handler as CustomTypeHandler;
    ...
    public bool IsSimpleType(Type type)
    {
        ...
        if (type != null)
        {
            var handler = GetTypeHandler(type, null);
