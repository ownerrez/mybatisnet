...
public class SelectStrategy : IArgumentStrategy
{
    ...
    {
        var paramString = mapping.ColumnName;
        object keys = null;
        var wasNull = false;
        ...
        if (paramString.IndexOf(',') > 0 || paramString.IndexOf('=') > 0) // composite parameters key
        {
            ...
            var paramTab = paramString.Split(splitter);
            if (paramTab.Length % 2 != 0) throw new DataMapperException("Invalid composite key string format in '" + mapping.PropertyName + ". It must be: property1=column1,property2=column2,...");
            var enumerator = paramTab.GetEnumerator();
            while (!wasNull && enumerator.MoveNext())
            {
                var hashKey = ((string)enumerator.Current).Trim();
                if (paramString.Contains("=")) // old 1.x style multiple params
                    enumerator.MoveNext();
                var hashValue = reader.GetValue(reader.GetOrdinal(((string)enumerator.Current).Trim()));
