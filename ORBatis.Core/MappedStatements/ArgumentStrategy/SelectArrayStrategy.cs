...
public sealed class SelectArrayStrategy : IArgumentStrategy
{
        ...
        ref IDataReader reader, object keys)
    {
        ...
        var values = selectStatement.ExecuteQueryForList(request.Session, keys);

        var elementType = mapping.MemberType.GetElementType();
        var array = Array.CreateInstance(elementType, values.Count);
        var count = values.Count;
        for (var i = 0; i < count; i++) array.SetValue(values[i], i);
        return array;
    }
    #endregion
}