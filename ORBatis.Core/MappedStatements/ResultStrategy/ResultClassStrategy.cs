...
public sealed class ResultClassStrategy : IResultStrategy
{
    ...
    public object Process(RequestScope request, ref IDataReader reader, object resultObject)
    {
        ...
        if (request.DataExchangeFactory.TypeHandlerFactory.IsSimpleType(request.CurrentResultMap.Class)) return _simpleTypeStrategy.Process(request, ref reader, resultObject);

        if (typeof(IDictionary).IsAssignableFrom(request.CurrentResultMap.Class)) return _dictionaryStrategy.Process(request, ref reader, resultObject);

        if (typeof(IList).IsAssignableFrom(request.CurrentResultMap.Class)) return _listStrategy.Process(request, ref reader, resultObject);
