...
internal class InlineParameterMapParser
{
    ...
    public SqlText ParseInlineParameterMap(IScope scope, IStatement statement, string sqlStatement)
    {
        ...
        var mappingList = new ArrayList();
        Type parameterClassType = null;

        if (statement != null) parameterClassType = statement.ParameterClass;

        var parser = new StringTokenizer(sqlStatement, PARAMETER_TOKEN, true);
        var newSqlBuffer = new StringBuilder();
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (PARAMETER_TOKEN.Equals(lastToken))
            {
                ...
                else
                {
                    ...
                    enumerator.MoveNext();
                    token = (string)enumerator.Current;
                    if (!PARAMETER_TOKEN.Equals(token)) throw new DataMapperException("Unterminated inline parameter in mapped statement (" + statement.Id + ").");
                    token = null;
                }
            }
            else
    ...
    {
        var mapping = new ParameterProperty();

        var paramParser = new StringTokenizer(token, "=,", false);
        var enumeratorParam = paramParser.GetEnumerator();
        ...
        {
            var field = (string)enumeratorParam.Current;
            if (enumeratorParam.MoveNext())
            {
                var value = (string)enumeratorParam.Current;
    ...
    private ParameterProperty OldParseMapping(string token, Type parameterClassType, IScope scope)
    {
        ...
        if (token.IndexOf(PARAM_DELIM) > -1)
        {
            var paramParser = new StringTokenizer(token, PARAM_DELIM, true);
            var enumeratorParam = paramParser.GetEnumerator();

            var n1 = paramParser.TokenNumber;
            if (n1 == 3)
            {
                enumeratorParam.MoveNext();
                var propertyName = ((string)enumeratorParam.Current).Trim();
                mapping.PropertyName = propertyName;

                enumeratorParam.MoveNext();
                enumeratorParam.MoveNext(); //ignore ":"
                var dBType = ((string)enumeratorParam.Current).Trim();
            ...
            else if (n1 >= 5)
            {
                ...
                enumeratorParam.MoveNext();
                enumeratorParam.MoveNext(); //ignore ":"
                var nullValue = ((string)enumeratorParam.Current).Trim();
                while (enumeratorParam.MoveNext()) nullValue = nullValue + ((string)enumeratorParam.Current).Trim();
        ...
        string propertyType, string dbType)
    {
        ...
        else if (typeof(IDictionary).IsAssignableFrom(parameterClassType))
        {
            if (propertyType == null || propertyType.Length == 0)
                ...
                try
                {
                    var typeClass = TypeUtils.ResolveType(propertyType);
        ...
        {
            var typeClass = ObjectProbe.GetMemberTypeForGetter(parameterClassType, propertyName);
            handler = typeHandlerFactory.GetTypeHandler(typeClass, dbType);
        }
