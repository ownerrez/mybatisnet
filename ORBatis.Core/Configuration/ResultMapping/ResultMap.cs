...
public class ResultMap : IResultMap
{
    ...
    public ResultMap(ConfigurationScope configScope, string id, string className, string extendMap, string groupBy)
    {
        ...
        if (id == null || id.Length < 1) throw new ArgumentNullException("The id attribute is mandatory in a ResultMap tag.");
        _id = configScope.ApplyNamespace(id);
        if (className == null || className.Length < 1) throw new ArgumentNullException("The class attribute is mandatory in the ResultMap tag id:" + _id);
        _className = className;
        _extendMap = extendMap;
        if (groupBy != null && groupBy.Length > 0)
        {
            var groupByProperties = groupBy.Split(',');
            for (var i = 0; i < groupByProperties.Length; i++)
            {
                var memberName = groupByProperties[i].Trim();
    ...
    [NonSerialized] private StringCollection _groupByPropertyNames = new();

    [NonSerialized] private ResultPropertyCollection _properties = new();

    [NonSerialized] private ResultPropertyCollection _groupByProperties = new();

    [NonSerialized] private ResultPropertyCollection _parameters = new();

    [NonSerialized] private Discriminator _discriminator;

    [NonSerialized] private string _sqlMapNameSpace = string.Empty;

    [NonSerialized] private IFactory _objectFactory;

    [NonSerialized] private DataExchangeFactory _dataExchangeFactory;
    ...
    public void Initialize(ConfigurationScope configScope)
    {
        try
        {
            ...
            for (var i = 0; i < _groupByProperties.Count; i++)
            {
                var memberName = GroupByPropertyNames[i];
    ...
    {
        for (var i = 0; i < GroupByPropertyNames.Count; i++)
        {
            var resultProperty = Properties.FindByPropertyName(GroupByPropertyNames[i]);
    ...
    private void GetChildNode(ConfigurationScope configScope)
    {
        ...
        var nodeList = configScope.NodeContext.SelectNodes(DomSqlMapBuilder.ApplyMappingNamespacePrefix(XML_CONSTRUCTOR_ARGUMENT), configScope.XmlNamespaceManager);
        if (nodeList.Count > 0)
        {
            var parametersType = new Type[nodeList.Count];
            var parametersName = new string[nodeList.Count];
            for (var i = 0; i < nodeList.Count; i++)
            {
                var argumentMapping = ArgumentPropertyDeSerializer.Deserialize(nodeList[i], configScope);
                _parameters.Add(argumentMapping);
                parametersName[i] = argumentMapping.ArgumentName;
            }

            var constructorInfo = GetConstructor(_class, parametersName);
            for (var i = 0; i < _parameters.Count; i++)
            {
                var argumentMapping = (ArgumentProperty)_parameters[i];
        ...
        {
            if (Type.GetTypeCode(_class) == TypeCode.Object) _objectFactory = configScope.SqlMapper.ObjectFactory.CreateFactory(_class, Type.EmptyTypes);
        }
        #endregion
        ...
        var discriminatorNode = configScope.NodeContext.SelectSingleNode(DomSqlMapBuilder.ApplyMappingNamespacePrefix(XML_DISCRIMNATOR), configScope.XmlNamespaceManager);
        ...
        if (configScope.NodeContext.SelectNodes(DomSqlMapBuilder.ApplyMappingNamespacePrefix(XML_SUBMAP), configScope.XmlNamespaceManager).Count > 0 && Discriminator == null) throw new ConfigurationException("The discriminator is null, but somehow a subMap was reached.  This is a bug.");
    ...
    {
        var parametersType = new Type[_parameters.Count];
        for (var i = 0; i < _parameters.Count; i++)
        {
            var argumentMapping = (ArgumentProperty)_parameters[i];
            parametersType[i] = argumentMapping.MemberType;
    ...
    {
        var candidates = type.GetConstructors(ANY_VISIBILITY_INSTANCE);
        foreach (var constructor in candidates)
        {
            ...
            if (parameters.Length == parametersName.Length)
            {
                ...
                {
                    var ok = parameters[j].Name == parametersName[j];
                    if (!ok)
                    {
                        found = false;
                        break;
    ...
    {
        var typeCode = Type.GetTypeCode(_class);

        if (typeCode == TypeCode.Object) return _objectFactory.CreateInstance(parameters);
    ...
    public IResultMap ResolveSubMap(IDataReader dataReader)
    {
        ...
        {
            var mapping = _discriminator.ResultProperty;
            var dataBaseValue = mapping.GetDataBaseValue(dataReader);
