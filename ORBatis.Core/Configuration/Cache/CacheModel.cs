...
public class CacheModel
{
    ...
    {
        var controllerId = HashCodeProvider.GetIdentityHashCode(_controller);
        var keyHash = key.GetHashCode();
        var lockKey = 29 * controllerId + keyHash;
        var lok = _lockMap[lockKey];
        if (lok == null)
        {
    ...
    [NonSerialized] private static readonly ILog _logger = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);

    /// <summary>
    ///     This is used to represent null objects that are returned from the cache so
    ///     that they can be cached, too.
    /// </summary>
    [NonSerialized] public static readonly object NULL_OBJECT = new();
    ...
    [NonSerialized] private object _statLock = new();
    ...
    [NonSerialized] private HybridDictionary _properties = new();
    ...
    public void Initialize()
    {
        ...
        try
        {
            if (_implementation == null) throw new DataMapperException("Error instantiating cache controller for cache named '" + _id + "'. Cause: The class for name '" + _implementation + "' could not be found.");

            // Build the CacheController
            var type = TypeUtils.ResolveType(_implementation);
    ...
    {
        if (_logger.IsDebugEnabled) _logger.Debug("Flush cacheModel named " + _id + " for statement '" + e.StatementName + "'");
    ...
    public object this[CacheKey key]
    {
        get
        {
            ...
            // No longer supported in Net8.0++. Strictly forbidden by microsoft.
        ...
        set
        {
            ...
            if (_logger.IsDebugEnabled) _logger.Debug(string.Format("Cache object '{0}' using key '{1}' ", value, key));
        }
    }
    ...
    public double HitRatio
    {
        get
        {
            if (_requests != 0) return _hits / (double)_requests;
