...
namespace IBatisNet.Common.Utilities.Objects
{
    ...
    public sealed class ReflectionInfo
    {
        ...
        private ReflectionInfo(Type type)
        {
            ...
            var getArray = new string[_getMembers.Count];
            _getMembers.Keys.CopyTo(getArray, 0);
            _readableMemberNames = getArray;

            var setArray = new string[_setMembers.Count];
        ...
        {
            #region Properties
            var properties = type.GetProperties(BINDING_FLAGS_PROPERTY);
            ...
            #region Fields
            var fields = type.GetFields(BINDING_FLAGS_FIELD);
        ...
        {
            var memberInfo = (MemberInfo)_setMembers[memberName];

            if (memberInfo == null) throw new ProbeException("There is no Set member named '" + memberName + "' in class '" + ClassName + "'");
        ...
        {
            var memberInfo = _getMembers[memberName] as MemberInfo;
            if (memberInfo == null) throw new ProbeException("There is no Get member named '" + memberName + "' in class '" + ClassName + "'");
            return memberInfo;
        }
        ...
        {
            var type = (Type)_setTypes[memberName];
            if (type == null) throw new ProbeException("There is no Set member named '" + memberName + "' in class '" + ClassName + "'");
            return type;
        }
        ...
        {
            var type = (Type)_getTypes[memberName];
            if (type == null) throw new ProbeException("There is no Get member named '" + memberName + "' in class '" + ClassName + "'");
            return type;
        }
        ...
        public static ReflectionInfo GetInstance(Type type)
        {
            lock (type)
            {
                var cache = (ReflectionInfo)_reflectionInfoMap[type];
                if (cache == null)
                {
