    ...
    public sealed class ObjectProbe
    {
        ...
        public static Type GetMemberTypeForSetter(object obj, string memberName)
        {
            ...
            else
            {
                ...
                {
                    var parser = new StringTokenizer(memberName, ".");
            ...
            AccessorFactory accessorFactory)
        {
            ...
            try
            {
                var startIndex = indexedName.IndexOf("[");
                var length = indexedName.IndexOf("]");
                var name = indexedName.Substring(0, startIndex);
                var index = indexedName.Substring(startIndex + 1, length - (startIndex + 1));
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var getAccessorFactory = accessorFactory.GetAccessorFactory;
                        var getAccessor = getAccessorFactory.CreateGetAccessor(targetType, memberName);

                        if (getAccessor == null) throw new ProbeException("No Get method for member " + memberName + " on instance of " + obj.GetType().Name);
                        try
                        {
            ...
            AccessorFactory accessorFactory)
        {
            if (memberName.IndexOf('.') > -1)
            {
                ...
                var currentPropertyName = (string)enumerator.Current;
                var child = obj;

                while (enumerator.MoveNext())
                {
                    var type = GetMemberTypeForSetter(child, currentPropertyName);
                    var parent = child;
                    child = GetMember(parent, currentPropertyName, accessorFactory);
                    if (child == null)
                        try
                        {
                            var factory = objectFactory.CreateFactory(type, Type.EmptyTypes);
            ...
            AccessorFactory accessorFactory)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        var targetType = obj.GetType();
                        var setAccessorFactory = accessorFactory.SetAccessorFactory;
                        var setAccessor = setAccessorFactory.CreateSetAccessor(targetType, memberName);

                        if (setAccessor == null) throw new ProbeException("No Set method for member " + memberName + " on instance of " + obj.GetType().Name);
                        try
                        {
        ...
        public static bool HasWritableProperty(object obj, string propertyName)
        {
            ...
            else
            {
                ...
                {
                    var parser = new StringTokenizer(propertyName, ".");
                    var enumerator = parser.GetEnumerator();
                    var type = obj.GetType();
        ...
        {
            if (_simpleTypeMap.Contains(type)) return true;
