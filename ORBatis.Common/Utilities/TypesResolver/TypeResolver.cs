...
namespace IBatisNet.Common.Utilities.TypesResolver
{
    ...
    public class TypeResolver : ITypeResolver
    {
        ...
        {
            var type = ResolveGenericType(typeName.Replace(" ", string.Empty));
            if (type == null) type = ResolveType(typeName.Replace(" ", string.Empty));
            return type;
        }
        #endregion
        ...
        private Type ResolveGenericType(string typeName)
        {
            ...
            if (typeName.StartsWith(NULLABLE_TYPE)) return null;

            var genericInfo = new GenericArgumentsInfo(typeName);
            Type type = null;
            try
            {
                if (genericInfo.ContainsGenericArguments)
                {
                    ...
                    if (!genericInfo.IsGenericDefinition)
                    {
                        ...
                        for (var i = 0; i < unresolvedGenericArgs.Length; i++) genericArgs[i] = TypeUtils.ResolveType(unresolvedGenericArgs[i]);
        ...
        internal class GenericArgumentsInfo
        {
            ...
            {
                // Check for match
                var isMatch = generic.IsMatch(originalString);
                if (!isMatch)
                {
                ...
                {
                    var argsStartIndex = originalString.IndexOf(GENERIC_ARGUMENTS_PREFIX);
                    var argsEndIndex = originalString.LastIndexOf(GENERIC_ARGUMENTS_SUFFIX);
                    if (argsEndIndex != -1)
                    {
                        SplitGenericArguments(originalString.Substring(
                            argsStartIndex + 1, argsEndIndex - argsStartIndex));

                        GenericTypeName = originalString.Remove(argsStartIndex, argsEndIndex - argsStartIndex + 2);
                    }
                }
            }
            ...
            private void SplitGenericArguments(string originalArgs)
            {
                ...
                {
                    var argument = originalArgs.Substring(1, originalArgs.Length - 2).Trim();
        ...
        internal class TypeAssemblyInfo
        {
            ...
            private void SplitTypeAndAssemblyNames(string originalTypeName)
            {
                ...
                {
                    var typeAssemblyIndex = originalTypeName.IndexOf(NULLABLE_TYPE_ASSEMBLY_SEPARATOR);
                ...
                {
                    var typeAssemblyIndex = originalTypeName.IndexOf(TYPE_ASSEMBLY_SEPARATOR);
                    if (typeAssemblyIndex < 0)
                    {
                        TypeName = originalTypeName;
                    }
