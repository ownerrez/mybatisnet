...
namespace IBatisNet.Common.Utilities
{
    ...
    public sealed class DBHelperParameterCache
    {
            ...
            bool includeReturnValueParameter)
        {
            ...
            using (var connection = session.DataSource.DbProvider.CreateConnection())
            {
                ...
                using (var cmd = connection.CreateCommand())
                {
                    ...
                    {
                        var firstParameter = (IDataParameter)cmd.Parameters[0];
        ...
        private IDataParameter[] CloneParameters(IDataParameter[] originalParameters)
        {
            ...
            for (int i = 0, j = length; i < j; i++) clonedParameters[i] = (IDataParameter)((ICloneable)originalParameters[i]).Clone();
        ...
        public IDataParameter[] GetCachedParameterSet(string connectionString, string commandText)
        {
            ...
            var cachedParameters = (IDataParameter[])_paramCache[hashKey];
        ...
        {
            var hashKey = session.DataSource.ConnectionString + ":" + spName + (includeReturnValueParameter ? ":include ReturnValue Parameter" : "");
