...
public class InMemoryDataReader : IDataReader
{
    ...
    private class InMemoryResultSet
    {
        ...
        private readonly StringDictionary _fieldsNameLookup = new();
            ...
            while (isMidstream || reader.Read())
            {
                if (firstRow)
                    for (var fieldIndex = 0; fieldIndex < reader.FieldCount; fieldIndex++)
                    {
                        ...
                        if (!_fieldsNameLookup.ContainsKey(fieldName)) _fieldsNameLookup.Add(fieldName, fieldIndex.ToString());
                ...
                var columnsValues = new object[FieldCount];
        ...
        {
            if (_fieldsNameLookup.ContainsKey(colName)) return Convert.ToInt32(_fieldsNameLookup[colName]);
    ...
    /// </summary>
    public int RecordsAffected => throw new NotImplementedException("InMemoryDataReader only used for select IList statements !");
    ...
    public long GetBytes(int fieldIndex, long dataIndex, byte[] buffer, int bufferIndex, int length)
    {
        ...
        var availableLength = (int)(((byte[])value).Length - dataIndex);
