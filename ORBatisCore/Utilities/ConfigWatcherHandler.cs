...
public sealed class ConfigWatcherHandler
{
    ...
    {
        for (var index = 0; index < _filesToWatch.Count; index++)
        {
            var configFile = (FileInfo)_filesToWatch[index];
    ...
    /// </summary>
    private static readonly ArrayList _filesToWatch = new();

    /// <summary>
    ///     The list of FileSystemWatcher.
    /// </summary>
    private static readonly ArrayList _filesWatcher = new();
    ...
    public static void ClearFilesMonitored()
    {
        ...
        for (var index = 0; index < _filesWatcher.Count; index++)
        {
            var fileWatcher = (FileSystemWatcher)_filesWatcher[index];
    ...
    {
        if (_logger.IsDebugEnabled) _logger.Debug("ConfigWatcherHandler : " + e.ChangeType + " [" + e.OldName + "/" + e.Name + "]");
