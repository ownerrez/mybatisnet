...
public sealed class DelegatePropertyGetAccessor : BaseAccessor, IGetAccessor
{
    ...
    public DelegatePropertyGetAccessor(Type targetObjectType, string propertyName)
    {
        ...
        var propertyInfo = GetPropertyInfo(targetObjectType);
        ...
        {
            var dynamicMethod = new DynamicMethod("GetImplementation", typeof(object), new[] { typeof(object) }, GetType().Module, true);
            ...
            if (targetGetMethod.ReturnType.IsValueType) ilgen.Emit(OpCodes.Box, targetGetMethod.ReturnType); //Box if necessary
    ...
    {
        if (_canRead) return _get(target);
