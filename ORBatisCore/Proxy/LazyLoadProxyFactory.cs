...
public class LazyLoadProxyFactory : ILazyFactory
{
    ...
    {
        var typeProxified = setAccessor.MemberType;

        if (logger.IsDebugEnabled) logger.Debug(string.Format("Statement '{0}', create proxy for member {1}.", selectStatement.Id, setAccessor.MemberType));
        ...
        var generatedProxy = proxyGenerator.CreateClassProxy(typeProxified, Type.EmptyTypes, interceptor);
