...
public class LazyFactoryBuilder
{
    ...
    public ILazyFactory GetLazyFactory(Type type)
    {
        if (type.IsInterface)
        {
            if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(IList<>)) return _factory[type.GetGenericTypeDefinition()] as ILazyFactory;

            if (type == typeof(IList)) return _factory[type] as ILazyFactory;
