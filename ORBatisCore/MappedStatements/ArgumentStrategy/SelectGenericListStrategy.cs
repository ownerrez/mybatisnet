...
public sealed class SelectGenericListStrategy : IArgumentStrategy
{
        ...
        ref IDataReader reader, object keys)
    {
        ...
        var selectStatement = request.MappedStatement.SqlMap.GetMappedStatement(mapping.Select);

        reader = DataReaderTransformer.Transform(reader, request.Session.DataSource.DbProvider);

        var typeArgs = mapping.MemberType.GetGenericArguments();
        var genericList = typeof(IList<>);
        var constructedType = genericList.MakeGenericType(typeArgs);
        var elementType = mapping.MemberType.GetGenericArguments()[0];

        var mappedStatementType = selectStatement.GetType();

        Type[] typeArguments = { typeof(SqlMapSession), typeof(object) };

        var mis = mappedStatementType.GetMethods(BindingFlags.InvokeMethod | BindingFlags.Public | BindingFlags.Instance);
        MethodInfo mi = null;
        ...
        var miConstructed = mi.MakeGenericMethod(elementType);

        // Invoke the method.
        object[] args = { request.Session, keys };
        var values = miConstructed.Invoke(selectStatement, args);
