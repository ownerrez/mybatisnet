...
public class MappedStatement : IMappedStatement
{
    ...
    private void RetrieveOutputParameters(RequestScope request, ISqlMapSession session, IDbCommand command, object result)
    {
        ...
        {
            var count = request.ParameterMap.PropertiesList.Count;
            for (var i = 0; i < count; i++)
            {
                var mapping = request.ParameterMap.GetProperty(i);
                    ...
                    mapping.Direction == ParameterDirection.InputOutput)
                {
                    ...
                    if (mapping.TypeHandler == null) // Find the TypeHandler
                        lock (mapping)
                        {
                            ...
                            {
                                var propertyType = ObjectProbe.GetMemberTypeForGetter(result, mapping.PropertyName);
                    ...
                    var dbValue = dataParameter.Value;

                    object value = null;

                    var wasNull = dbValue == DBNull.Value;
                    if (wasNull)
                    {
    ...
    public dynamic ExecuteQueryForDynamicObject(ISqlMapSession session, object parameterObject)
    {
        ...
        var obj = RunQueryForDynamicObject(request, session, parameterObject);
    ...
    {
        var resultObject = default(dynamic);
        ...
        using (var command = request.IDbCommand)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (obj != BaseStrategy.SKIP) resultObject = obj;
                }
            }
            finally
            {
    ...
    public virtual IDictionary ExecuteQueryForMapWithRowDelegate(ISqlMapSession session, object parameterObject, string keyProperty, string valueProperty, DictionaryRowDelegate rowDelegate)
    {
        ...
        if (rowDelegate == null) throw new DataMapperException("A null DictionaryRowDelegate was passed to QueryForMapWithRowDelegate.");
        ...
        using (var command = request.IDbCommand)
        {
            ...
            try
            {
                if (rowDelegate == null)
                    ...
                    {
                        var obj = _resultStrategy.Process(request, ref reader, null);
                        var key = ObjectProbe.GetMemberValue(obj, keyProperty, request.DataExchangeFactory.AccessorFactory);
                        var value = obj;
                        if (valueProperty != null) value = ObjectProbe.GetMemberValue(obj, valueProperty, request.DataExchangeFactory.AccessorFactory);
                        map.Add(key, value);
                    }
                else
                    while (reader.Read())
    ...
    private void ExecutePostSelect(RequestScope request)
    {
        ...
        {
            var postSelect = request.QueueSelect.Dequeue() as PostBindind;
