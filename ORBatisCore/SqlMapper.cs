...
public class SqlMapper : ISqlMapper
{
        ...
        AccessorFactory accessorFactory)
    {
        ...
        DBHelperParameterCache = new DBHelperParameterCache();
        _objectFactory = objectFactory;
        AccessorFactory = accessorFactory;

        DataExchangeFactory = new DataExchangeFactory(TypeHandlerFactory, _objectFactory, accessorFactory);
    ...
    private readonly HybridDictionary _cacheMaps = new();
    ...
    public DBHelperParameterCache DBHelperParameterCache { get; }
    ...
    {
        if (MappedStatements.Contains(key)) throw new DataMapperException("This SQL map already contains a MappedStatement named " + mappedStatement.Id);
    ...
    /// </summary>
    public HybridDictionary MappedStatements { get; } = new();
    ...
    {
        if (ParameterMaps.Contains(parameterMap.Id)) throw new DataMapperException("This SQL map already contains an ParameterMap named " + parameterMap.Id);
    ...
    {
        if (ResultMaps.Contains(resultMap.Id)) throw new DataMapperException("This SQL map already contains an ResultMap named " + resultMap.Id);
    ...
    /// </summary>
    public HybridDictionary ParameterMaps { get; } = new();

    /// <summary>
    ///     The ResultMap collection
    /// </summary>
    public HybridDictionary ResultMaps { get; } = new();
    ...
    {
        var enumerator = _cacheMaps.GetEnumerator();
    ...
    {
        if (_cacheMaps.Contains(cache.Id)) throw new DataMapperException("This SQL map already contains an Cache named " + cache.Id);
    ...
    {
        if (!_cacheMaps.Contains(name)) throw new DataMapperException("This SQL map does not contain an Cache named " + name);
    ...
    public string GetDataCacheStats()
    {
        ...
        var enumerator = MappedStatements.GetEnumerator();
        while (enumerator.MoveNext())
        {
            var mappedStatement = (IMappedStatement)enumerator.Value;
            ...
            {
                var hitRatio = ((CachingStatement)mappedStatement).GetDataCacheHitRatio();
                if (hitRatio != -1)
                {
