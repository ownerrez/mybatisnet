...
public class ParameterMap
{
    ...
    [NonSerialized]
    // Properties list
    private ParameterPropertyCollection _properties = new();

    // Same list as _properties but without doubled (Test UpdateAccountViaParameterMap2)
    [NonSerialized] private ParameterPropertyCollection _propertiesList = new();

    //(property Name, property)
    [NonSerialized] private Hashtable _propertiesMap = new(); // Corrected ?? Support Request 1043181, move to HashTable
    ...
    {
        var propertyNameArray = new string[_propertiesMap.Count];

        for (var index = 0; index < _propertiesList.Count; index++) propertyNameArray[index] = _propertiesList[index].PropertyName;
    ...
    {
        var value = _dataExchange.GetData(mapping, parameterValue);

        var typeHandler = mapping.TypeHandler;
