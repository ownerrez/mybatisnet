...
public class ParameterProperty
{
    ...
    {
        if (_directionAttribute.Length > 0) _direction = (ParameterDirection)Enum.Parse(typeof(ParameterDirection), _directionAttribute, true);
            ...
            && !scope.DataExchangeFactory.TypeHandlerFactory.IsSimpleType(parameterClass)) // value property
        {
            ...
            {
                var getAccessorFactory = scope.DataExchangeFactory.AccessorFactory.GetAccessorFactory;
            ...
            else // complex member name FavouriteLineItem.Id
            {
                ...
                var parentName = _propertyName.Substring(0, _propertyName.LastIndexOf('.'));
                var parentType = ObjectProbe.GetMemberTypeForGetter(parameterClass, parentName);
        ...
        if (CallBackName.Length > 0)
        {
            try
            {
                var type = scope.DataExchangeFactory.TypeHandlerFactory.GetType(CallBackName);
