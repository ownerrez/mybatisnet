...
public class MemoryCacheLevel
{
    ...
    #endregion
    #region Fields
    private static readonly Hashtable _cacheLevelMap = new();
    ...
    {
        var cacheLevel = (MemoryCacheLevel)_cacheLevelMap[referenceType];
        if (cacheLevel == null) throw new DataMapperException("Error getting CacheLevel (reference type) for name: '" + referenceType + "'.");
        return cacheLevel;
    }
    #endregion
}