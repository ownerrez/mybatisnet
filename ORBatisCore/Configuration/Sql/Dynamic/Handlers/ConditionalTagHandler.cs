...
public abstract class ConditionalTagHandler : BaseTagHandler
{
    ...
    {
        if (IsCondition(ctx, tag, parameterObject)) return INCLUDE_BODY;
    ...
    protected long Compare(SqlTagContext ctx, SqlTag sqlTag, object parameterObject)
    {
        ...
        var propertyName = tag.Property;
        var literal = tag.Literal;
        var comparePropertyName = tag.CompareProperty;
        var compareValue = tag.CompareValue;

        object value1 = null;
        Type type = null;
        if (propertyName != null && propertyName.Length > 0)
        {
            var iterate = ctx.PeekIterateContext();
        ...
        {
            var value2 = ObjectProbe.GetMemberValue(parameterObject, comparePropertyName, AccessorFactory);
            return CompareValues(type, value1, value2);
        }

        if (!string.IsNullOrEmpty(compareValue)) return CompareValues(type, value1, compareValue);
    ...
    protected object ConvertValue(Type type, string value)
    {
        ...
        if (type == typeof(char)) return Convert.ToChar(value.Substring(0, 1)); //new Character(value.charAt(0));
