...
internal sealed class SimpleDynamicSql : ISql
{
    ...
    private string ProcessDynamicElements(object parameterObject)
    {
        ...
        var parser = new StringTokenizer(_simpleSqlStatement, ELEMENT_TOKEN, true);
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (ELEMENT_TOKEN.Equals(lastToken))
            {
                ...
                else
                {
                    ...
                    if (!ELEMENT_TOKEN.Equals(token)) throw new DataMapperException("Unterminated dynamic element in sql (" + _simpleSqlStatement + ").");
                    token = null;
                }
            }
            else
    ...
    {
        var sqlStatement = ProcessDynamicElements(parameterObject);
