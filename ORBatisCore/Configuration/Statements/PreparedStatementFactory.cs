...
public class PreparedStatementFactory
{
    ...
    public PreparedStatement Prepare()
    {
        ...
        else if (_statement.CommandType == CommandType.StoredProcedure) // StoredProcedure
        {
            ...
            {
                var commandTextBuilder = new StringBuilder("{ call ");
                commandTextBuilder.Append(_commandText);

                if (_preparedStatement.DbParameters.Length > 0)
                {
                    commandTextBuilder.Append(" (");
                    var supIndex = _preparedStatement.DbParameters.Length - 1;
                    for (var i = 0; i < supIndex; i++) commandTextBuilder.Append("?,");
                    commandTextBuilder.Append("?) }");
                }
        ...
        if (_logger.IsDebugEnabled) _logger.Debug("Statement Id: [" + _statement.Id + "] Prepared SQL: [" + _preparedStatement.PreparedSql + "]");
    ...
    private readonly HybridDictionary _propertyDbParameterMap = new();
    ...
    private void DiscoverParameter(ISqlMapSession session)
    {
        ...
        var commandParameters = session.SqlMapper.DBHelperParameterCache.GetSpParameterSet(session, _commandText);

        _preparedStatement.DbParameters = new IDbDataParameter[commandParameters.Length];

        var start = session.DataSource.DbProvider.ParameterPrefix.Length;
        for (var i = 0; i < commandParameters.Length; i++)
        {
            var dataParameter = (IDbDataParameter)commandParameters[i];
        ...
        var sortedDbParameters = new IDbDataParameter[commandParameters.Length];
        for (var i = 0; i < _statement.ParameterMap.Properties.Count; i++) sortedDbParameters[i] = Search(session, _preparedStatement.DbParameters, _statement.ParameterMap.Properties[i], i);
        ...
        if (property.ColumnName.Length > 0)
        {
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                if (session.DataSource.DbProvider.UseParameterPrefixInParameter)
                    ...
                    {
                        var prefixLength = session.DataSource.DbProvider.ParameterPrefix.Length;
    ...
    private void CreateParametersForTextCommand()
    {
        ...
        var dbTypePropertyName = _session.DataSource.DbProvider.ParameterDbTypeProperty;
        var enumDbType = _session.DataSource.DbProvider.ParameterDbType;
        ...
        for (var i = 0; i < list.Count; i++)
        {
            ...
            var dataParameter = _session.CreateDataParameter();
            ...
            if (property.DbType != null && property.DbType.Length > 0)
                try
                {
                    ...
                    var dbType = Enum.Parse(enumDbType, property.DbType, true);
            ...
            if (_session.DataSource.DbProvider.SetDbParameterPrecision) dataParameter.Precision = property.Precision;
            ...
            if (_session.DataSource.DbProvider.UsePositionalParameters == false) _propertyDbParameterMap.Add(property, dataParameter);
        }
    }
    ...
    private void CreateParametersForProcedureCommand()
    {
        ...
        for (var i = 0; i < list.Count; i++)
        {
            ...
            var dataParameter = _session.CreateCommand(_statement.CommandType).CreateParameter();
    ...
    private void EvaluateParameterMap()
    {
        ...
        var index = 0;
        var sqlParamName = string.Empty;
        var parser = new StringTokenizer(_commandText, delimiter, true);
        var newCommandTextBuffer = new StringBuilder();
        ...
        while (enumerator.MoveNext())
        {
            ...
            {
                var property = _request.ParameterMap.Properties[index];
            ...
            else
            {
                ...
                var endPos = token.LastIndexOf('#');

                if (startPos != endPos)
                {
                    var name = token.Substring(startPos + 1, endPos - startPos - 1);

                    if (_session.DataSource.DbProvider.UsePositionalParameters)
                        throw new InvalidOperationException("Can't use parameter name reference with positional parameters.");

                    var property = _request.ParameterMap.GetProperty(name);
