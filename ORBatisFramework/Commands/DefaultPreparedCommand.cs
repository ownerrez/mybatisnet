...
namespace IBatisNet.DataMapper.Commands
{
    ...
    internal class DefaultPreparedCommand : IPreparedCommand
    {
        ...
        public void Create(RequestScope request, ISqlMapSession session, IStatement statement, object parameterObject)
        {
            ...
            if (_logger.IsDebugEnabled) _logger.Debug("Statement Id: [" + statement.Id + "] PreparedStatement : [" + request.IDbCommand.CommandText + "]");
        ...
        {
            var properties = request.PreparedStatement.DbParametersName;
            var parameters = request.PreparedStatement.DbParameters;
            var paramLogList = new StringBuilder(); // Log info
            var typeLogList = new StringBuilder(); // Log info
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                var property = request.ParameterMap.GetProperty(i);
                ...
                if (request.ParameterMap != null)
                    ...
                    {
                        var dbTypePropertyName = session.DataSource.DbProvider.ParameterDbTypeProperty;
                        var propertyValue = ObjectProbe.GetMemberValue(sqlParameter, dbTypePropertyName, request.DataExchangeFactory.AccessorFactory);
                ...
                if (session.DataSource.DbProvider.SetDbParameterPrecision) parameterCopy.Precision = sqlParameter.Precision;
