...
namespace IBatisNet.DataMapper.MappedStatements
{
    ...
    public sealed class CachingStatement : IMappedStatement
    {
        ...
        {
            if (_mappedStatement.Statement.CacheModel != null) return _mappedStatement.Statement.CacheModel.HitRatio;
        ...
        private CacheKey GetCacheKey(RequestScope request)
        {
            ...
            var count = request.IDbCommand.Parameters.Count;
            for (var i = 0; i < count; i++)
            {
                var dataParameter = (IDataParameter)request.IDbCommand.Parameters[i];
            ...
            var cacheModel = _mappedStatement.Statement.CacheModel;
        ...
        {
            return _mappedStatement.ExecuteQueryForDictionary(session, parameterObject, keyProperty, valueProperty, rowDelegate);
        }
        #endregion
        ...
        public T ExecuteQueryForObject<T>(ISqlMapSession session, object parameterObject, T resultObject)
        {
            ...
            var cacheObjet = Statement.CacheModel[cacheKey];
            // check if this query has alreay been run 
            if (cacheObjet is T)
            {
                obj = (T)cacheObjet;
