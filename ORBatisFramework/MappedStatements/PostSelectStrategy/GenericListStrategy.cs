...
namespace IBatisNet.DataMapper.MappedStatements.PostSelectStrategy
{
    ...
    public sealed class GenericListStrategy : IPostSelectStrategy
    {
        ...
        public void Execute(PostBindind postSelect, RequestScope request)
        {
            ...
            var typeArgs = postSelect.ResultProperty.SetAccessor.MemberType.GetGenericArguments();
            var genericList = typeof(IList<>);
            var constructedType = genericList.MakeGenericType(typeArgs);
            var elementType = postSelect.ResultProperty.SetAccessor.MemberType.GetGenericArguments()[0];

            var mappedStatementType = postSelect.Statement.GetType();
            ...
            var values = miConstructed.Invoke(postSelect.Statement, args);
