...
namespace IBatisNet.DataMapper.MappedStatements.ResultStrategy
{
    ...
    public sealed class GroupByStrategy : BaseStrategy, IResultStrategy
    {
        ...
        public object Process(RequestScope request, ref IDataReader reader, object resultObject)
        {
            ...
            var resultMap = request.CurrentResultMap.ResolveSubMap(reader);

            var uniqueKey = GetUniqueKey(resultMap, request, reader);
            // Gets the [key, result object] already build
            var buildObjects = request.GetUniqueKeys(resultMap);
            ...
            if (buildObjects != null && buildObjects.Contains(uniqueKey))
            {
                ...
                {
                    var resultProperty = resultMap.Properties[index];
                    if (resultProperty.PropertyStrategy is PropertStrategy.GroupByStrategy) resultProperty.PropertyStrategy.Set(request, resultMap, resultProperty, ref outObject, reader, null);
                }
