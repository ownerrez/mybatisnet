...
namespace IBatisNet.DataMapper.MappedStatements.PropertStrategy
{
    ...
    public sealed class GroupByStrategy : BaseStrategy, IPropertyStrategy
    {
        ...
        public object Get(RequestScope request, IResultMap resultMap, ResultProperty mapping, ref object target, IDataReader reader)
        {
            ...
            var property = ObjectProbe.GetMemberValue(target, mapping.PropertyName,
            ...
            object result = null;
            var propertyRresultMap = mapping.NestedResultMap.ResolveSubMap(reader);

            if (propertyRresultMap.GroupByProperties.Count > 0)
            {
                var uniqueKey = GetUniqueKey(propertyRresultMap, request, reader);

                // Gets the [key, result object] already build
                var buildObjects = request.GetUniqueKeys(propertyRresultMap);
                ...
                if (buildObjects != null && buildObjects.Contains(uniqueKey))
                {
                    ...
                    if (result != null)
                        ...
                        for (var index = 0; index < propertyRresultMap.Properties.Count; index++)
                        {
                            var resultProperty = propertyRresultMap.Properties[index];
                            if (resultProperty.PropertyStrategy is GroupByStrategy) resultProperty.PropertyStrategy.Set(request, propertyRresultMap, resultProperty, ref result, reader, null);
                        }
