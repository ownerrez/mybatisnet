...
namespace IBatisNet.DataMapper.MappedStatements.PropertyStrategy
{
    ...
    public sealed class SelectGenericListStrategy : IPropertyStrategy
    {
            ...
            ResultProperty mapping, ref object target, IDataReader reader, object keys)
        {
            ...
            var postSelect = new PostBindind();
            ...
            {
                var values = mapping.LazyFactory.CreateProxy(selectStatement, keys, target, mapping.SetAccessor);
                mapping.SetAccessor.Set(target, values);
            }
            else
            {
                if (mapping.SetAccessor.MemberType.GetGenericTypeDefinition() == typeof(IList<>)) postSelect.Method = PostBindind.ExecuteMethod.ExecuteQueryForGenericIList;
