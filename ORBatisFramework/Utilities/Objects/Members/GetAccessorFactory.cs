...
namespace IBatisNet.Common.Utilities.Objects.Members
{
    ...
    public class GetAccessorFactory : IGetAccessorFactory
    {
        ...
        public IGetAccessor CreateGetAccessor(Type targetType, string name)
        {
            ...
            lock (_syncObject)
            {
                if (!_cachedIGetAccessor.Contains(key))
                {
                    ...
                    var memberInfo = reflectionCache.GetGetter(name);
        ...
        {
            var reflectionCache = ReflectionInfo.GetInstance(targetType);
            var propertyInfo = (PropertyInfo)reflectionCache.GetGetter(propertyName);
        ...
        {
            var reflectionCache = ReflectionInfo.GetInstance(targetType);
            var fieldInfo = (FieldInfo)reflectionCache.GetGetter(fieldName);

            if (fieldInfo.IsPublic) return new DelegateFieldGetAccessor(targetType, fieldName);
        ...
        private IGetAccessor CreateFieldAccessor(Type targetType, string fieldName)
        {
            ...
            if (fieldInfo.IsPublic) return new EmitFieldGetAccessor(targetType, fieldName, _assemblyBuilder, _moduleBuilder);
