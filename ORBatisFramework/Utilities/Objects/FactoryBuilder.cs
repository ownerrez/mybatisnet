...
namespace IBatisNet.Common.Utilities.Objects
{
    ...
    public class FactoryBuilder
    {
        ...
        public FactoryBuilder()
        {
            ...
            var _assemblyBuilder = AssemblyBuilder.DefineDynamicAssembly(assemblyName, AssemblyBuilderAccess.Run);
        ...
        public IFactory CreateFactory(Type typeToCreate, Type[] types)
        {
            ...
            {
                if (_logger.IsInfoEnabled) _logger.Info("Create a stub IFactory for abstract type " + typeToCreate.Name);
                return new AbstractFactory(typeToCreate);
            }

            var innerType = CreateFactoryType(typeToCreate, types);
            var ctor = innerType.GetConstructor(Type.EmptyTypes);
        ...
        private Type CreateFactoryType(Type typeToCreate, Type[] types)
        {
            ...
            for (var i = 0; i < types.Length; i++) typesName += types[i].Name.Replace("[]", string.Empty) + i;
            var typeBuilder = _moduleBuilder.DefineType("EmitFactoryFor" + typeToCreate.FullName + typesName, TypeAttributes.Public);
        ...
        private void ImplementCreateInstance(TypeBuilder typeBuilder, Type typeToCreate, Type[] argumentTypes)
        {
            ...
            var meth = typeBuilder.DefineMethod("CreateInstance", CREATE_METHOD_ATTRIBUTES, typeof(object), new[] { typeof(object[]) });
            var il = meth.GetILGenerator();

            // Add test if contructeur not public
            var ctor = typeToCreate.GetConstructor(VISIBILITY, null, argumentTypes, null);
