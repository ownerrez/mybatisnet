...
namespace IBatisNet.DataMapper.Configuration.ParameterMapping
{
    ...
    public class ParameterPropertyCollection
    {
        ...
        {
            for (var i = 0; i < value.Count; i++) Add(value[i]);
        }
        ...
        {
            var oldSize = _innerList.Length;

            if (minSize > oldSize)
            {
                var oldEntries = _innerList;
                var newSize = oldEntries.Length * CAPACITY_MULTIPLIER;
