...
namespace IBatisNet.DataMapper.Configuration.ResultMapping
{
    ...
    public class ResultProperty
    {
        ...
        public virtual Type MemberType
        {
            get
            {
                if (_setAccessor != null) return _setAccessor.MemberType;

                if (_nestedResultMap != null) return _nestedResultMap.Class;
        ...
        public void Initialize(ConfigurationScope configScope, Type resultClass)
        {
                ...
                !typeof(IDictionary).IsAssignableFrom(resultClass))
            {
                ...
                {
                    var propertyInfo = ObjectProbe.GetMemberInfoForSetter(resultClass, _propertyName);
                    var memberName = _propertyName.Substring(_propertyName.LastIndexOf('.') + 1);
                ...
                if (_isGenericIList)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var genericIList = typeof(IList<>);
                            var interfaceListType = genericIList.MakeGenericType(typeArgs);

                            var genericList = typeof(List<>);
                            var listType = genericList.MakeGenericType(typeArgs);
            ...
            if (CallBackName != null && CallBackName.Length > 0)
            {
                ...
                try
                {
                    var type = configScope.SqlMapper.TypeHandlerFactory.GetType(CallBackName);
                    var typeHandlerCallback = (ITypeHandlerCallback)Activator.CreateInstance(type);
        ...
        public object GetDataBaseValue(IDataReader dataReader)
        {
            ...
            var wasNull = value == DBNull.Value;
            if (wasNull)
            {
                if (HasNullValue)
                {
        ...
        {
            if (value == null) return TypeHandler.NullValue;
