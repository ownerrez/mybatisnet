...
namespace IBatisNet.DataMapper.Proxy
{
    ...
    public class LazyListGenericFactory : ILazyFactory
    {
        ...
        {
            var elementType = setAccessor.MemberType.GetGenericArguments()[0];
            var lazyType = typeof(LazyListGeneric<>);
            var lazyGenericType = lazyType.MakeGenericType(elementType);
