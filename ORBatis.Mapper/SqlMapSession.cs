...
namespace IBatisNet.DataMapper
{
    ...
    public class SqlMapSession : ISqlMapSession
    {
        ...
        public void CloseConnection()
        {
            ...
            {
                _connection.Close();
                if (_logger.IsDebugEnabled) _logger.Debug(string.Format("Close Connection \"{0}\" to \"{1}\".", _connection.GetHashCode().ToString(), DataSource.DbProvider.Description));
                _connection.Dispose();
            }
        ...
        public void BeginTransaction(string connectionString, bool openConnection, IsolationLevel isolationLevel)
        {
            ...
            {
                if (_connection == null || _connection.State != ConnectionState.Open) throw new DataMapperException("SqlMapSession could not invoke StartTransaction(). A Connection must be started. Call OpenConnection() first.");
                _transaction = _connection.BeginTransaction(isolationLevel);
                if (_logger.IsDebugEnabled) _logger.Debug("Begin Transaction.");
                IsTransactionStart = true;
            }
        ...
        public void CommitTransaction()
        {
            ...
            if (_connection != null && _connection.State != ConnectionState.Closed) CloseConnection();
        }
