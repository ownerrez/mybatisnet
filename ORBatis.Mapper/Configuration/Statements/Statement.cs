...
namespace IBatisNet.DataMapper.Configuration.Statements
{
    ...
    public class Statement : IStatement
    {
        ...
        [NonSerialized] private ResultMapCollection _resultsMap = new ResultMapCollection();
        ...
        internal virtual void Initialize(ConfigurationScope configurationScope)
        {
            ...
            {
                var names = _resultMapName.Split(',');
                for (var i = 0; i < names.Length; i++)
                {
                    var name = configurationScope.ApplyNamespace(names[i].Trim());
                    _resultsMap.Add(configurationScope.SqlMapper.GetResultMap(name));
                }
            }

            if (_parameterMapName.Length > 0) _parameterMap = configurationScope.SqlMapper.GetParameterMap(_parameterMapName);
            if (_resultClassName.Length > 0)
            {
                var classNames = _resultClassName.Split(',');
                for (var i = 0; i < classNames.Length; i++)
                {
                    ...
                    var dataExchange = configurationScope.DataExchangeFactory.GetDataExchangeForClass(_resultClass);
                    IResultMap autoMap = new AutoResultMap(_resultClass, resultClassFactory, dataExchange);
                    _resultsMap.Add(autoMap);
                }
            }

            if (_parameterClassName.Length > 0) _parameterClass = configurationScope.SqlMapper.TypeHandlerFactory.GetType(_parameterClassName);
