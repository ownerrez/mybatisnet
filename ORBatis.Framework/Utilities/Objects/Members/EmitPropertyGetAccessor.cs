...
namespace IBatisNet.Common.Utilities.Objects.Members
{
    ...
    public sealed class EmitPropertyGetAccessor : BaseAccessor, IGetAccessor
    {
        ...
        public EmitPropertyGetAccessor(Type targetObjectType, string propertyName, AssemblyBuilder assemblyBuilder, ModuleBuilder moduleBuilder)
        {
            ...
            var propertyInfo = _targetType.GetProperty(propertyName, BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
        ...
        {
            if (_canRead) return _emittedGet.Get(target);
        ...
        private void EmitType(ModuleBuilder moduleBuilder)
        {
            ...
            var typeBuilder = moduleBuilder.DefineType("GetFor" + _targetType.FullName + _propertyName,
            ...
            var methodBuilder = typeBuilder.DefineMethod("Get",
            ...
            var generatorIL = methodBuilder.GetILGenerator();

            if (_canRead)
            {
                // Emit the IL for get access. 
                var targetGetMethod = _targetType.GetMethod("get_" + _propertyName, BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
                ...
                if (targetGetMethod.ReturnType.IsValueType) generatorIL.Emit(OpCodes.Box, targetGetMethod.ReturnType); //Box if necessary
